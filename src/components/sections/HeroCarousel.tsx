          ...
          >
            ...
            <div className="absolute inset-0 overflow-hidden">
              <img
                src={slide.imageUrl}
                alt={slide.title}
                className={`w-full h-full object-cover opacity-60 transition-transform duration-[10000ms] ease-out origin-center transform ${isActive ? 'scale-110' : 'scale-100'
                  }`}
              />
            </div>
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-4 pt-16">
              ...
              {/* Title */}
              <h2
                className={`text-4xl md:text-7xl lg:text-8xl font-serif font-bold text-white mb-6 drop-shadow-xl transition-all duration-1000 delay-300 ease-out transform ${isActive ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0'
              ...
              {/* Subtitle */}
              <p
                className={`text-xl md:text-2xl lg:text-3xl text-gray-100 font-light drop-shadow-md max-w-3xl mb-10 tracking-wide transition-all duration-1000 delay-500 ease-out transform ${isActive ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0'
                  }`}
              >
                {slide.subtitle}
              </p>
              ...
              {/* Button */}
              <div
                className={`transition-all duration-1000 delay-700 ease-out transform ${isActive ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0'
                  }`}
              >
      ...
      <div className="absolute bottom-12 left-1/2 -translate-x-1/2 flex space-x-4 z-20">
        {slides.map((_, index) => (
          <button
            ...
            className={`h-1 transition-all duration-500 ease-out ${index === currentSlide ? 'bg-af-gold w-16' : 'bg-white/40 hover:bg-white w-8'
              }`}
          />
        ))}
      </div>
