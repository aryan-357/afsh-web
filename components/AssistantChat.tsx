...
const AssistantChat: React.FC = () => {
  ...
  return (
    <>
      ...
      {/* Chat Window */}
      {isOpen && (
        <div className="fixed bottom-24 right-6 w-[90vw] md:w-[380px] h-[500px] bg-white rounded-2xl shadow-2xl z-50 flex flex-col border border-gray-200 animate-fade-in-up overflow-hidden">
          {/* Header */}
            ...
            {isLoading && (
              <div className="flex justify-start">
                <div className="bg-white rounded-2xl rounded-bl-none px-4 py-3 shadow-sm border border-gray-100">
                  <Loader2 className="w-5 h-5 animate-spin text-af-blue" />
                </div>
              </div>
            )}
          ...
          <div className="p-4 bg-white border-t border-gray-100">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-gray-100 rounded-full px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-af-blue/50 text-gray-700"
                disabled={isLoading}
              />
              <button
                onClick={handleSend}
